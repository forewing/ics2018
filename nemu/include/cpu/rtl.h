...
static inline void rtl_not(rtlreg_t *dest, const rtlreg_t* src1) {
  // dest <- ~src1
  rtl_host_lm(&at, src1, 4);
  at = ~at;
  rtl_host_sm(dest, &at, 4);
  // TODO();
}
...
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- src1[width * 8 - 1]
  rtl_li(&at, (*src1 >> (width*8-1)) & 1);
  rtl_host_sm(dest, &at, 4);
}

static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- signext(src1[(width * 8 - 1) .. 0])
  rtl_msb(&at, src1, width);
  switch(at){
    case 0:
      switch(width){
        case 4:
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  cpu.ZF = ((*result & (0xffffffff >> ((4-width)*8))) == 0);
}
...
static inline void rtl_get_cmpl(rtlreg_t* dest, const rtlreg_t* src, int width){
  *dest = *src ^ 0xffffffff;
  *dest += 1;
  *dest &= (0xffffffff >> ((4-width)*8));
}
